...
void AllGatherGetCollByteCount(size_t *sendcount, size_t *recvcount, size_t *tempcount, size_t *paramcount, size_t *sendInplaceOffset, size_t *recvInplaceOffset, size_t *tempInplaceOffset, size_t count, int nranks) {
  *sendcount = count/nranks;
  *recvcount = (count/nranks)*nranks;
  *tempcount = (count/nranks)*nranks;
...
testResult_t AllGatherRunColl(void* sendbuff, void* recvbuff, void* tempbuff, size_t count, ncclDataType_t type, ncclRedOp_t op, int root, ncclComm_t comm, cudaStream_t stream) {
  NCCLCHECK(ncclAllGather(sendbuff, recvbuff, tempbuff, count, type, comm, stream));
  return testSuccess;
}
...
void AllGatherGetBuffSize(size_t *sendcount, size_t *recvcount, size_t *tempcount, size_t count, int nranks) {
  size_t paramcount, sendInplaceOffset, recvInplaceOffset, tempInplaceOffset;
  AllGatherGetCollByteCount(sendcount, recvcount, tempcount, &paramcount, &sendInplaceOffset, &recvInplaceOffset, &tempInplaceOffset, count, nranks);
}
