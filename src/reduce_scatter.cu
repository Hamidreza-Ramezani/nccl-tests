...
void ReduceScatterGetCollByteCount(size_t *sendcount, size_t *recvcount, size_t *tempcount, size_t *paramcount, size_t *sendInplaceOffset, size_t *recvInplaceOffset, size_t *tempInplaceOffset, size_t count, int nranks) {
  *sendcount = (count/nranks)*nranks;
  *recvcount = count/nranks;
  *tempcount = count/nranks;
  *sendInplaceOffset = 0;
  *recvInplaceOffset = count/nranks;
  *tempInplaceOffset = count/nranks;
...
testResult_t ReduceScatterRunColl(void* sendbuff, void* recvbuff, void* tempbuff, size_t count, ncclDataType_t type, ncclRedOp_t op, int root, ncclComm_t comm, cudaStream_t stream) {
  NCCLCHECK(ncclReduceScatter(sendbuff, recvbuff, tempbuff, count, type, op, comm, stream));
  return testSuccess;
}
...
void ReduceScatterGetBuffSize(size_t *sendcount, size_t *recvcount, size_t *tempcount, size_t count, int nranks) {
  size_t paramcount, sendInplaceOffset, recvInplaceOffset, tempInplaceOffset;
  ReduceScatterGetCollByteCount(sendcount, recvcount, tempcount, &paramcount, &sendInplaceOffset, &recvInplaceOffset, &tempInplaceOffset, count, nranks);
}
